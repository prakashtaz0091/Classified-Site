...
def google_login(request):
    try:
        ...
        CLIENT_ID = (
            "230604273167-guv4saijfahagueh5kc6ggq01mug2ilv.apps.googleusercontent.com"
        )
        idinfo = id_token.verify_oauth2_token(
            id_token_data, requests.Request(), CLIENT_ID
        )
        ...
        print(request.user)
        return JsonResponse(
            {"status": "success", "message": "User login successfully", "email": email},
            status=200,
        )
        ...
        print(error)
        return JsonResponse(
            {"message": f"unexpected error {error}", "status": False}, status=400
        )
